...
namespace uv
{
	...
	public:
		...
		inline int		close(uv_file file, std::function<void()> handler);
		inline int		open(const std::string &path, int flags, int mode, std::function<void()> handler);
		inline int		read(uv_file file, const uv_buf_t bufs[], unsigned int nbufs, int64_t offset, std::function<void()> handler);
		inline int		write(uv_file file, const uv_buf_t bufs[], unsigned int nbufs, int64_t offset, std::function<void()> handler);
		inline int		unlink(const std::string &path, std::function<void()> handler);
		inline int		mkdir(const std::string &path, int mode, std::function<void()> handler);
		inline int		mkdtemp(const std::string &tpl, std::function<void()> handler);
		inline int		rmdir(const std::string &path, std::function<void()> handler);
		inline int		scandir(const std::string &path, int flags, std::function<void()> handler);
		inline int		scandirNext(uv_dirent_t* ent);
		inline int		stat(const std::string &path, std::function<void()> handler);
		...
		inline int		utime(const std::string &path, double atime, double mtime, std::function<void()> handler);
		inline int		futime(uv_file file, double atime, double mtime, std::function<void()> handler);
		inline int		lstat(const std::string &path, std::function<void()> handler);
		inline int		link(const std::string &path, const std::string &newPath, std::function<void()> handler);
		inline int		symlink(const std::string &path, const std::string &newPath, int flags, std::function<void()> handler);
		inline int		readlink(const std::string &path, std::function<void()> handler);
		inline int		realpath(const std::string &path, std::function<void()> handler);
		inline int		fchmod(uv_file file, int mode, std::function<void()> handler);
		inline int		chown(const std::string &path, uv_uid_t uid, uv_gid_t gid, std::function<void()> handler);
		inline int		fchown(uv_file file, uv_uid_t uid, uv_gid_t gid, std::function<void()> handler);
	private:
		uv::Loop			&m_loop;
